...
structure_type <- 'alternative2'

# print(structure_type)
# 
# cells <- readRDS(paste(here('DATA/splittedCells_'), gsub(' ','_', gsub('+','plus',structure_type,fixed=T),fixed = T), '.rds',sep=''))

# For region analysis:
# cells <- cells %>% rename(ImageNumber = Split_ImageNumber)
...
write_delim(AUCScaledSlides_300 %>% 
              mutate(phenotype_combo=paste(phenotype_from,phenotype_to,sep='_to_')), paste(here('scratch/AUCScaledSlides_300_'),gsub(' ', '_', gsub('+','plus',structure_type,fixed=T), fixed=T) ,'.tsv', sep=''),delim='\t')

toc()

combinations <- expand.grid(unique(AUCScaledSlides_300$phenotype_from), unique(AUCScaledSlides_300$phenotype_to)) %>% unite(combination, Var1:Var2,sep = '_to_') %>% pull(combination)
saveRDS(combinations, file=here('scratch/combinations_selection_run3.rds'))
