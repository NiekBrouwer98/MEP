...
distance_data <- paste(here('scratch/AUCScaledSlides_300_'),gsub(' ', '_', gsub('+','plus',structure_type,fixed=T), fixed=T) ,'.tsv', sep='')
initial_params_folder <- paste(here('scratch/init_parameters_'),gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T), sep = '')

cells <- read_fst(here('DATA/SingleCells_altered.fst'))
# cells <- readRDS(paste(here('DATA/splittedCells_'), gsub(' ','_', gsub('+','plus',structure_type,fixed=T),fixed = T), '.rds',sep='')) %>% dplyr::select(-c(ImageNumber)) %>% dplyr::rename(ImageNumber = Split_ImageNumber)
outfile <- paste(here('scratch/success_models_'),gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T),sep='')
...
combinations <- readRDS(here('scratch/combinations_selection_run3.rds'))

# Directory for results
# Directory for intermediate results
if (!dir.exists(paste(here('scratch/success_models_'),gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T), sep = ''))){
  dir.create(paste(here('scratch/success_models_'),gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T), sep = ''))
}else{
  message("dir exists")
}

finished_files <- list.files(paste(here('scratch/success_models_'),gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T),'/', sep = ''))
