...
distance_data <- here(paste('scratch/AUCScaledSlides_300_', gsub(' ', '_',gsub('+','plus',structure_type,fixed=T),fixed=T), '.tsv',sep=''))
outfile <- paste(here('scratch/init_parameters_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T),sep='')

combinations <- readRDS(here('scratch/combinations_selection_run3.rds'))


# Directory for intermediate results
if (!dir.exists(paste(here('scratch/init_parameters_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T), sep = ''))){
  dir.create(paste(here('scratch/init_parameters_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T), sep = ''))
}else{
  message("dir exists")
}

finished_files <- list.files(paste(here('scratch/init_parameters_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T),'/', sep = ''))
...
#   }))
# 
# saveRDS(all_combos_dists, file = paste(here('scratch/all_combos_dists_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T),'.rds', sep = ''))
print('first step complete')



# Use dataset generated by code above
all_combos_dists <- readRDS(paste(here('scratch/all_combos_dists_'),gsub(' ', '_',gsub('+','plus',structure_type, fixed=T),fixed=T),'.rds', sep = ''))

# Run over phenotype ccombinations in parallel
initial_parameters <-mclapply(combinations_selection, mc.cores = 16, function(c){
  message(c)
