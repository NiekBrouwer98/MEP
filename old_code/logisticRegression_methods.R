...
predictorInitialFile <- function(){
  ...
  colnames(cellPhenotypes)[colnames(cellPhenotypes) == "Basal"] <- "Basal_cells"  
...
getAUC <- function(
  ...
  save_rds_archive(coefList, file=paste(here('scratch/coeflists_run3/coefList_'), gsub('+', 'plus', responseVar, fixed=T), '_', predictor_name, '.rds', sep=''),last_modified = T, with_time = T)
  ...
  # save_rds_archive(roc, file= paste(here('scratch/ROCS_run3/roc_'),responseVar, '_', predictor_name, '.rds', sep=''), last_modified = T, with_time = T)
...
computeEstimationPercentages <- function(){
