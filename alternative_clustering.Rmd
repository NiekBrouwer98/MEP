...
hierarchical_clustering <- function(df, subtype,cluster_n, name){
  ...
  p <- Heatmap(name = subtype, t(as.matrix(df)), cluster_columns = T,cluster_rows=T, column_names_gp = gpar(fontsize=2), row_names_gp = gpar(fontsize=8), column_split = cut_avg, col = heatmapColorScale(-4,4))
  save_pdf(p, paste(here('output/Method_comparison/heatmaps/'), paste(gsub('+','plus',subtype,fixed=T),t,sep='_'), 'HCHM.pdf',sep=''))
...
getFeatures <- function(subtype,type){
  ...
  originalDensity_features <- grep('_originalType', type_subset, fixed = T, value = T)
  # originalDensity_features <- gsub("[^A-Za-z0-9+\\-\\_\\]", "", originalDensity_features)

  distance_features <- c(grep('shape', type_subset, fixed = T, value = T),grep('scale', type_subset, fixed = T, value = T) )
  density_features <- setdiff(type_subset, c(originalDensity_features,distance_features))
  
  
  if (type == 'originalDensity'){
    return(originalDensity_features)
  }
  if (type == 'density'){
