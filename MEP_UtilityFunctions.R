...
show_slide <- function(sample_number, highlight_A){
  slide_1 <- ggplot(data=cells%>% filter(ImageNumber == sample_number) %>% mutate(type = ifelse(meta_description %in% highlight_A, meta_description, 'other'))) + geom_point(aes(x=Location_Center_X, y=Location_Center_Y, color=type), alpha =0.5) +
...
show_distance_distribution <- function(sample){
...
transform_parameters_to_matrix <- function(){
...
getCombinationCounts <- function(){
  ...
  expand_cell_combinations <- cell_counts %>% tidyr::expand(tnumber, meta_description, meta_description) %>% 
