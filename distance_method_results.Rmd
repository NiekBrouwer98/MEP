...
g1 <- ggplot() +geom_bar(data = results_melted %>% filter(experiment == 'original_experiment'), aes(x=factor(category, level=c('all', 'existing', '20to5', '20to20')), y=value, fill=variable),stat="itdentity", position='fill')+ scale_y_continuous(labels = scales::percent_format()) + xlab('') + ylab('') + ggtitle('Original classification') + theme_bw() + theme(legend.position = "none")
...
library(circlize)
col_fun = colorRamp2(c(0, 100, 794), c("black", "white", "red"))
...
hm1 <- Heatmap(all_parameters_matrix, cluster_rows = T, cluster_columns = T, col=col_fun, column_names_gp = grid::gpar(fontsize = 6),
  row_names_gp = grid::gpar(fontsize = 6),column_title = 'phenotype_from', row_title = 'phenotype_to')
save_pdf(hm1, filename = here('output/Distance_results/Missing_estimations_analysis/allestimatedHM2.pdf'),width=7,height=6)
...
hm1 <- Heatmap(all_parameters_matrix_alternative, cluster_rows = T, cluster_columns = T, col=col_fun, column_names_gp = grid::gpar(fontsize = 6),
  row_names_gp = grid::gpar(fontsize = 6),column_title = 'phenotype_from', row_title = 'phenotype_to')
save_pdf(hm1, filename = here('output/Distance_results/alternative_classification/allestimatedHM2_alternative.pdf'),width=7,height=6)
...
failedcombinationsAlt <- cellCombinationCountsAlt %>% filter(unique_sample %in% setdiff(cellCombinationCountsAlt %>% pull(unique_sample), all_parametersAlt %>% pull(unique_sample)))  %>% filter(n_to > 5 & n_from > 20)
...
Take a look at the distributions with shape = 3.

```{r}
three_line <- all_parametersAlt %>% filter(round(shape,5) == 3.09034)
distance_data_line <- cellCombinationCountsAlt %>% filter(unique_sample %in% (three_line %>% pull(unique_sample))) %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample)

ggplot() + 
  geom_point(data = all_parametersAlt %>% filter(!(unique_sample %in% (three_line %>% pull(unique_sample)))), aes(x=shape, y=scale, colour='all'),,alpha=0.1, size=0.5) +
  geom_point(data = three_line, aes(x=shape, y=scale,colour='line'), alpha=0.5, size=0.5) +
  theme_bw() +
  ylim(10,300) + xlim(0,6) +
  scale_y_log10() +
  xlab('Shape') + ylab('Scale') + ggtitle("All parameters with outliers") + 
...
sample_line <- sample_n(three_line %>% filter(!(unique_sample %in% outliersAlt)) %>% filter(unique_sample %in% distance_data_line),10)

for (row in 1:nrow(sample_line)){
  slide <- show_distance_distribution(cellsAlt, all_distances_dataAlt, sample_line[row,],0.2)
  print(slide)
}

```
```{r}
initParams_line2 <- reshape2::dcast(`init_params_CK+_ERorHER2+_cells_to_CD38+_cells`, combo + tnumber ~ term, value.var = 'estimate')

ggplot() +
  # geom_point(data =initParams_line, aes(x=shape, y=scale),size=0.5, alpha=1) +
  geom_point(data =initParams_line2, aes(x=shape, y=scale),colour='red', alpha=0.5,size=0.5, alpha=1) +
  theme_bw() +
  ylim(10,300) + xlim(0,6) +
  scale_y_log10() +
  xlab('Shape') + ylab('Scale') + ggtitle("init parameters")

```



The line are estimations for the combination: CK+_ERorHER2+_cells_to_CD38+_cells.
