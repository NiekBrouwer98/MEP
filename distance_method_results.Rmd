...
```{r}
all_possible_samples <- cell_occurences_combinations %>% filter(n_to >= 5 & n_from >= 20)
...
all_parameters_counts <- merge(all_parameters_counts, all_possibilities_counts %>% select(c(phenotype_combo,n)), all.x=T, by='phenotype_combo') %>% mutate(n_est_p = (n_estimated/(n+1))*100)

ggplot(data=(all_possibilities_counts %>% mutate(n_p = (n/794) * 100))) + geom_tile(aes(x=phenotype_from, y=phenotype_to, fill=n_p)) + geom_text(aes(x=phenotype_from, y=phenotype_to, label=n),size=1,color='white') +   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
ggsave(here('output/Distance_results/Missing_estimations_analysis/allpossibleHM.pdf'),width=7,height=6)

ggplot(data=(all_parameters_counts)) + geom_tile(aes(x=phenotype_from, y=phenotype_to, fill=n_est_p)) + geom_text(aes(x=phenotype_from, y=phenotype_to, label=n_estimated),size=1,color='white') +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave(here('output/Distance_results/Missing_estimations_analysis/estimatedHM.pdf'),width=7,height=6)
...
How many valid estimations where made? (with criteria minimum 20 from, 5 to cells)

```{r}
length(intersect(cell_occurences_combinations %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample), all_parameters  %>% pull(unique_sample)))/length(cell_occurences_combinations %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample))*100
```
We are not necessarily interested in cell types. Take a look at the interesting combinations.

```{r}
tumor_cells <- setdiff(getTumorAndTMETypes()$tumor_types, c("Ep Ki67^{+}","CK^{lo}ER^{lo}"))
stromal_cells <- setdiff(getTumorAndTMETypes()$TME_types, c("Ki67^{+}", "Macrophages & granulocytes", "CD4^{+} T cells & APCs" ))

length(
  intersect(cell_occurences_combinations %>% filter(phenotype_from %in% c(tumor_cells, stromal_cells) & phenotype_to %in% c(tumor_cells, stromal_cells)) %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample), all_parameters %>% filter(phenotype_from %in% c(tumor_cells, stromal_cells) & phenotype_to %in% c(tumor_cells, stromal_cells))  %>% pull(unique_sample)))/
  length(cell_occurences_combinations %>% filter(phenotype_from %in% c(tumor_cells, stromal_cells) & phenotype_to %in% c(tumor_cells, stromal_cells)) %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample))*100

combination_selection <-  intersect(cell_occurences_combinations %>% filter(phenotype_from %in% c(tumor_cells, stromal_cells) & phenotype_to %in% c(tumor_cells, stromal_cells)) %>% filter(n_from > 20 & n_to > 5) %>% pull(unique_sample), all_parameters %>% filter(phenotype_from %in% c(tumor_cells, stromal_cells) & phenotype_to %in% c(tumor_cells, stromal_cells))  %>% pull(unique_sample))
...
all_parameters_withMedianDifferences <- merge(all_parameters, medians %>% select(c(unique_sample, difference)), by='unique_sample', all.x=T)  %>% arrange(difference)

ggplot() +
  geom_point(data = all_parameters_withMedianDifferences, aes(x=shape, y=scale,colour=difference),size=0.5, alpha=1) +
  scale_colour_continuous() +
  scale_colour_gradient2(midpoint=25, mid='black',low="white", high="red", space ="Lab", name='delta median' ) +
  theme_bw() +
  ylim(10,300) + xlim(0,6) +
  scale_y_log10() +
  xlab('Shape') + ylab('Scale') + ggtitle("Parameters with median difference")

ggsave(paste(here('output/Distance_results/Alberto_reproduction/'), 'parameters_withdeltamedian.png', sep=''),width=6,heigh=4)
...
ggsave(paste(here('output/Distance_results/Alberto_reproduction/'), 'allparameters_withCombinationMeans.pdf', sep=''))

ggplot() +
  geom_point(data = all_parameters, aes(x=shape, y=scale),color='black', alpha=0.1, size=0.5) +
  geom_point(data = means_images, aes(x=mean_shape, y=mean_scale),color='red',alpha = 0.5, size=1) +
  geom_text(data= means_images %>% filter((mean_shape > mean(means$mean_shape) + 2*sd(means$mean_shape)) | (mean_shape < mean(means$mean_shape) - 2*sd(means$mean_shape)) | (mean_scale > mean(means$mean_scale) + 2*sd(means$mean_scale) | (mean_scale < mean(means$mean_scale) - 2*sd(means$mean_scale)))), aes(x=mean_shape,y=mean_scale,label=tnumber),color='red',size=4) +
  theme_bw() +
  # labs(color=NULL) + 
  ylim(10,300) + xlim(0,6) +
  scale_y_log10() +
...
ggsave(paste(here('output/Distance_results/Alberto_reproduction/'), 'allparameters_withSampleMeans.pdf', sep=''))
...
```
The belly is explained by tumor to TME cell relationships.
```{r}
library(ggforce)

all_parameters_withColoring <- all_parameters %>% mutate(color_from = ifelse(phenotype_from %in% getTumorAndTMETypes()$TME_types,'TME','Tumor'))
all_parameters_withColoring <- all_parameters_withColoring  %>% mutate(color_to = ifelse(phenotype_to %in% getTumorAndTMETypes()$TME_types,'TME','Tumor'))
all_parameters_withColoring <- all_parameters_withColoring  %>% mutate(color_to = ifelse(phenotype_to == phenotype_from,'self',color_to))
all_parameters_withColoring <- all_parameters_withColoring  %>% mutate(color_from = ifelse(phenotype_to == phenotype_from,'self',color_from))
all_parameters_withColoring <- all_parameters_withColoring %>% unite('type',color_to:color_from, sep='_to_')

belly <- all_parameters_withColoring %>% filter((shape - 4)^2/3 + (scale - 70)^2/1450 <2 | (shape - 4)^2/3 + (scale - 120)^2/1000 <1.8)
point <- ggplot() +
  geom_point(data=all_parameters_withColoring, aes(x=shape, y=scale,colour='all'),color='grey',alpha=0.1, size=0.5) +
  geom_point(data = Alberto_parameters, aes(x=a, y=b),color= 'red', alpha = 0.5, size=0.5) +
    geom_point(data=belly, aes(x=shape, y=scale,colour=type),color = 'black', alpha=0.1, size=0.5) +
    ylim(10,300) + xlim(0,6) +
  theme_bw() + scale_y_log10() +
  xlab('Shape') + ylab('Scale')
...
occ <- belly %>% count(type)
occ_total <- all_parameters_withColoring %>% count(type)
occ_total <- merge(occ, occ_total, by='type', all.x=T)
occ_total <- occ_total %>% mutate(percentage = round((n.x / n.y),2))

occ <- belly %>% count(phenotype_combo)
occ_total <- all_parameters_withColoring %>% count(phenotype_combo)
occ_total <- merge(occ, occ_total, by='phenotype_combo', all.x=T)
occ_total <- occ_total %>% mutate(percentage = round((n.x / n.y),2))

# 
# for (i in unique(all_parameters_withColoring$type)){
#   point <- ggplot() +
#     geom_point(data=all_parameters_withColoring, aes(x=shape, y=scale,colour='all'),color='grey',alpha=0.1, size=0.5) +
#     geom_point(data=all_parameters_withColoring %>% filter(type == i), aes(x=shape, y=scale,colour=type),alpha=0.5, size=1) +
#     geom_density_2d(data = all_parameters_withColoring %>% filter(type == i), aes(x=shape, y=scale)) +
#   ylim(10,300) + xlim(0,6) +
#   theme_bw() + scale_y_log10() +
#   xlab('Shape') + ylab('Scale') +
#   theme(legend.position='bottom', legend.direction="vertical") + guides(colour = guide_legend(override.aes = list(size=10))) +
#   labs(color=NULL)
# print(point)
#  ggsave(paste(here('output/Distance_results/Alberto_reproduction/parameters_'), i, '.pdf',sep=''),height=8,width = 8)
#   
# }
