...
ggsave(here('output/boxplot_nphenotypes_metabric.jpg'))
...
ggsave(here('output/boxplot_nphenotypes_metabric2'))


```
Compare this to nabucco data:

```{r}
nabucco_data <- read_tsv(here('DATA/nabucco_1_densities.tsv'))

ggplot(nabucco_data) + geom_bar(aes(x=phenotype, y=n, fill=tnumber), stat="identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ggtitle("Number of cells per phenotype")

ggplot(nabucco_data, aes(x=phenotype, y=n))+
    geom_boxplot() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ggtitle("Number of cells per phenotype")

nabucco_data_stromal <- nabucco_data %>% filter(!phenotype %in% c('negative'))

ggplot(nabucco_data_stromal, aes(x=phenotype, y=n)) + coord_cartesian(ylim =  c(0, 7000)) + 
    geom_boxplot() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ggtitle("Number of cells per phenotype")

ggsave(here('output/boxplot_nphenotypes_nabucco.jpg'))
...
write_delim(AUCScaledSlides_300 %>% 
              mutate(phenotype_combo=paste(phenotype_from,phenotype_to,sep='_to_')), paste(here('scratch/AUCScaledSlides_300_ALL'), '.tsv', sep=''),delim='\t')
