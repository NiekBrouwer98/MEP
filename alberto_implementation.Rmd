...
#Data with sample number, cell index number, spatial location, phenotype and area
cells <- cells[,c("tnumber", "ObjectNumber", "Xcenter", "Ycenter", "phenotype", "AreaShape_Area")]

#For debugging we take a small subset, remove this when code is working
...
# phenotypes <- unique(cells$phenotype) #All phenotypes
phenotypes <- unique(phenotype_list$stromal_immune$pg_id_map$meta_description) #Only stromal immune phenotypes
...
```
Group 1-NN distances in bins.

```{r}
# Explore data from 0 to 300 microns 
# Modification for PanCK+: extend the maximum distance count further than 99 microns; do up to 300 microns
...
# LongBinCounts_300$Response <- nabresFiltered$two_class_pa_response[match(LongBinCounts_300$tnumber, nabresFiltered$tnumber)]
...
#Some groups only have 1 occurence, the normalized value is Inf in this cases, replace with 1
#Ask Alberto if he occured this problem, AUC function should be fixed
AUCScaledSlides_300$N.per.mm2.scaled[AUCScaledSlides_300$N.per.mm2.scaled == Inf] <- 1


#double check if the AUC of AUCs is 1
AUCcheck_panck <- AUCScaledSlides_300 %>%
  dplyr::group_by(phenotype_from, phenotype_to, tnumber) %>%
  dplyr::mutate(AUC = funAUC(WinMean, N.per.mm2.scaled))

#save this
# AUCScaledSlides_300_tumorandstroma$Response <- nabresFiltered$two_class_pa_response[match(AUCScaledSlides_300_tumorandstroma$tnumber, nabresFiltered$tnumber)]
...
write_delim(initial_params_weib, here('scratch/initial_params_weib.tsv'),delim='\t')
```
...
```{r NLME model}
initial_params <- initial_params_weib

#TODO Calculate number of cells for each cell type / sample (will be used later for filtering purposes)
...
#Missing clincial data from 3 blocks
