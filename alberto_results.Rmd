...
clinical_data <- merge(x= clinical_data, cells %>% select(c(ImageNumber, metabric_id)), by='metabric_id', all.x=T)
...
```
Take a look at the distance distributions of the combinations that could not be fitted.
...
plot_initial_parameters = function(t,c){
  ...
  return(ggplot(all_distances_data[all_distances_data$tnumber == t & all_distances_data$phenotype_combo == c ,]) +
           ...
           + labs(subtitle = paste('initial params: ', round(shape_param,2), ', ', round(scale_param,2))) + xlim(0,300)) + 
    theme(plot.subtitle = element_text(size=5,hjust = 0.5))
}
library(grid)

combination = not_estimated_combinations[1]
plts_notexistingparams <- lapply(sample(unique(all_distances_data %>% filter(phenotype_combo == combination) %>% pull(tnumber)), 6, replace=FALSE), plot_initial_parameters, c=not_estimated_combinations[1])
gridExtra::grid.arrange(grobs = plts_notexistingparams, ncol=2,
     top = textGrob(combination,gp=gpar(fontsize=12,font=2)))

combination = not_estimated_combinations[3]
plts_notexistingparams <- lapply(sample(unique(all_distances_data %>% filter(phenotype_combo == combination) %>% pull(tnumber)), 6, replace=FALSE), plot_initial_parameters, c=not_estimated_combinations[1])
gridExtra::grid.arrange(grobs = plts_notexistingparams, ncol=2,
     top = textGrob(combination,gp=gpar(fontsize=12,font=2)))
...
ggsave(paste(here('output/Alberto_results/'), 'notestimatedoccurences.png', sep=''))
...
ggsave(paste(here('output/Alberto_results/'), 'cellcounts_boxplot.png', sep=''))
...
ggsave(paste(here('output/Alberto_results/'), 'cellcounts_stackedbarplot.png', sep=''))
...
shape_parameters <- read_rds(here('scratch/shape_parameter_matrix.rds'))

# shape_parameters <- shape_parameters %>% replace(is.na(.), 0)
# scale_parameters <- scale_parameters %>% replace(is.na(.), 0)

shape_parameters_df <- as.data.frame(shape_parameters)
scale_parameters_df <- as.data.frame(scale_parameters)

rownames(shape_parameters_df) <- shape_parameters_df$tnumber
shape_parameters_df <- subset(shape_parameters_df, select=-c(tnumber))
rownames(scale_parameters_df) <- scale_parameters_df$tnumber
scale_parameters_df <- subset(scale_parameters_df, select=-c(tnumber))

# Scale scales columns
shape_parameters_df_scaled <- scale(shape_parameters_df) 
scale_parameters_df_scaled <- scale(scale_parameters_df) 


library(ComplexHeatmap)
library(tidyHeatmap)
ht <- Heatmap(shape_parameters_df_scaled, name = 'shape parameters', column_title = 'combinations', row_title = 'samples', na_col = 'black',
        row_names_gp = gpar(fontsize=2), column_names_gp = gpar(fontsize=2), cluster_rows = FALSE, cluster_columns =  FALSE)
save_pdf(ht, here('output/shape_heatmap.pdf'), width=20, height=12)

ht <- Heatmap(scale_parameters_df_scaled, name = 'scale parameters', column_title = 'combinations', row_title = 'samples',
        row_names_gp = gpar(fontsize=2), column_names_gp = gpar(fontsize=2), cluster_rows = FALSE)
save_pdf(ht, here('output/scale_heatmap.pdf'), width=20, height=12)
