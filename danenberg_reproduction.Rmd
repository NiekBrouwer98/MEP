...
## Data discovery
We reproduce the visualization of the cell phenotype counts and proportions as shown in extended figure 2 to show that the input datasets are the same.
...
# Hierarchical cluster order by count data
...
## Community detection
The TME is represented as a graph to use the community-detection algorithm to find densely connected cell clusters.

```{r community detection and property calculation}

# Run communiy-detection algorithm
...
# Retrieve network properties of individual communties for later analyses
...
## Cluster communities
For the detected communities, connectivity profiles are computed (phenotype composition and vertex degree).
...
TMEStructures[issues$VarDiff_TMEStructure$..ROWNUMBER..,]

```

Aggregate clusters per patient
...
toPlot$yaxisRev <- factor(toPlot$yaxisRev,
    levels = c("Suppressed expansion","TLS-like","Vascular stroma", "PDPN^{+} active stroma", "Active IR",
               "FSP1^{+} enriched","Granulocyte enriched","APC enriched","CD8^{+} & macrophages","Active stroma"),ordered = TRUE)
...
box <- ggplot() +
	...
	scale_fill_identity() +
	scale_x_discrete(labels = sortunique(toPlot$yaxisRev)) +
...
counts <- ggplot(data = toPlot[, .SD[1], by = yaxisRev]) +
	...
	facet_grid(cols = vars(countfacet), labeller = label_parsed) 
...
# facetlabs <- sapply(facetlabs, TeX)
...
legend_labels <- unlist(filllabs, use.names = FALSE)
