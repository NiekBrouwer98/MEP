...
print(paste('number of samples that cannot be estimated: ', nrow(cell_occurences_combinations %>% filter(n.x == 0 | n.y ==0))))
...
save_pdf(hm, here('output/data_exploration/zero_occurences_heatmap.pdf'), width=6, height=4)
...
save_pdf(hm, here('output/data_exploration/zeroplus_occurences_heatmap.pdf'), width=6, height=4)
...
save_pdf(hm, here('output/data_exploration/five_occurences_heatmap.pdf'), width=6, height=4)
...
save_pdf(hm, here('output/data_exploration/ten_occurences_heatmap.pdf'), width=6, height=4)
...
save_pdf(hm, here('output/data_exploration/twenty_occurences_heatmap.pdf'), width=6, height=4)
...
compute_from <- function(t){
   return(nrow(cell_occurences_combinations %>% filter(n.x >= t)))
}


compute_both <- function(t){
   return(nrow(cell_occurences_combinations %>% filter(n.x >= t & n.y >= t)))
}

n_from <- apply(thresholds,1, compute_from)
n_both <- apply(thresholds,1, compute_both)

thresholds <- cbind(thresholds, n_from, n_to, n_both)

ggplot(data=thresholds) + geom_point(aes(x=threshold, y=n_from, color = 'phenotype_from'))  + 
  geom_point(aes(x=threshold, y=n_both, color='both')) + theme_bw() + ggtitle('Number of samples with at least x cells') + ylab('count') + xlab('n cells') + xlim(0,50)
