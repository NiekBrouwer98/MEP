...
alternativeClasses = list(# Epithelial
                          # CK+ cells
                          'CK+_ERorHER2+_cells'  =  c( "ER^{hi}CXCL12^{+}" , "CK8-18^{+} ER^{hi}","HER2^{+}"),
                          'CK+_ERorHER2-_cells' = c("CK8-18^{hi}ER^{lo}","CK^{+} CXCL12^{+}","CK8-18^{hi}CXCL12^{hi}","Basal", "CD15^{+}","Ep CD57^{+}"), 
...
testMerge <- function(merge, remove){
  ...
  col_fun = colorRamp2(c(0, 100,101, 794), c("black",'black', "white", "red"))
  ...
  belowHunderedCount <- dfCounts %>% mutate(enoughSamples = ifelse(n > 100, T, F)) %>% group_by(phenotype_from) %>% count(enoughSamples) %>% filter(enoughSamples == F) %>% filter(n > round(length(unique(dfCounts$phenotype_from))/2)) %>% pull(phenotype_from)
