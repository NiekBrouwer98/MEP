...
cells <- readRDS(paste(here('DATA/splittedCells_'),structure_type,'.rds',sep='')) %>% dplyr::select(-c(ImageNumber)) %>% dplyr::rename(ImageNumber = Split_ImageNumber)
...
## cell type Densities
```{r}
densities <- merge(getCellCounts(cells),cells %>% group_by(ImageNumber) %>% summarise(max = max(Location_Center_X)), by.x='tnumber', by.y='ImageNumber')
densities <- merge(densities,cells %>% group_by(ImageNumber) %>% summarise(max = max(Location_Center_Y)), by.x='tnumber', by.y='ImageNumber')
densities <- densities %>% mutate(area = (max.x*max.y)/1000)
densities <- densities %>% mutate(density = n/area)
...
```{r}
cell_counts <- as_tibble(cells) %>% select(c(ImageNumber,metabric_id, meta_description)) %>%
...
istumour_labels <- (unique(cells %>% select(ImageNumber, isTumour)))
...
saveRDS(cellPhenotypes, file=paste(here('scratch/features/regions/cell_proportions_per_image_'),structure_type,'.rds',sep=''))
